...
raw correlator output is not. Eight of them have an exact source URL, recorded below; the rest
...
Twenty small files (30 358 810 B together) that VLBIFiles.jl has always tested against: the
...
test suite itself, so their URL is recorded in the code that fetched them; two more, the EHT
M87 image and the J0743-5619 map, were downloaded from the CDS catalogue and from astrogeo.org
respectively; the rest predate any such record and say so.
...
| `J0743-5619_S_2011_08_13_dew_map.fits` | 1 082 880 / `c26ea431800fda2e5ca91cdef73767e53d14f7c1` | [`astrogeo.org/images/J0743-5619/J0743-5619_S_2011_08_13_dew_map.fits`](https://astrogeo.org/images/J0743-5619/J0743-5619_S_2011_08_13_dew_map.fits) | 512² CLEAN image of J0743-5619, VLBA S-band 2011-08-13 (`v278b`, LBA Calibrator Survey `dew` pipeline), 1 mas pixels, **two `AIPS CC` tables** — `EXTVER = 1` with 507 components and `EXTVER = 2` with 25 — `img read two CC tables` |
...
One further fixture is referenced by the test suite under `pkg-data/` but is **not** published
here: `GOALS_GroupS2_Ref59_Cals_Avg.idi`, the Obit-written MeerKAT FITS-IDI, has no located
public source at all. Its testitem skips itself unless the file is dropped into `pkg-data/` by
hand, exactly as it did while the package still had a `test/data/`.

The two-CC-table case used to be served by `J0840-5732_X_2010_03_11_dew_map.fits`, which is
public — <https://astrogeo.org/images/J0840-5732/J0840-5732_X_2010_03_11_dew_map.fits>, SHA1
`1574f886f4339c4670086a980388cb7f685f9fa5` — but whose first of two `AIPS CC` tables holds
10 000 000 components, so the file is 124 217 280 B and exceeds GitHub's 100 MiB per-file limit.
`J0743-5619_S_2011_08_13_dew_map.fits` above is the same structure out of the same `dew`
pipeline at a hundredth of the size, so that testitem no longer skips and the J0840 map is not
needed by anything.
...
* **`pkg-data/`**, the fixtures moved out of the package repository. Where the source is known
  ...
  **astrogeo.org** (`J1256-0547_X_2020_10_18_pet_vis.fits`,
  `J0743-5619_S_2011_08_13_dew_map.fits`, credit the archive as above);
